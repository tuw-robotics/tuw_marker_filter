...
public:
    ...
    /**
     * starts the SLAM cycle and predicts the vehicles and landmark poses at the timestamp
     ...
     * @param zt measurments at time t
     **/
    virtual void cycle ( std::vector<Pose2D> &yt, cv::Mat_<double> &C_Yt, const Command &ut, const MeasurementConstPtr &zt ) = 0;
protected:
    static std::map<Type, std::string> TypeName_;           /// map of type enum to human readable names
     ...
     *
     * @return true on succsessful update, false on first use and if t is in the past
     **/
