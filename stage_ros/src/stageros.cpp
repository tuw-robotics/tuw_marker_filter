...
#define BASE_LASER_LINK "base_laser_link"
...
public:
    ...
    // to the first 'laser' model, the first 'position' model and the first
    // 'fiducial' model.  Returns 0 on success (both models subscribed),
    // -1 otherwise.
    int SubscribeModels();
    ...
    for (size_t r = 0; r < this->positionmodels.size(); r++)
    {
        ...
        for (size_t s = 0;  s < new_robot->fiducialmodels.size(); ++s)
        {
            if (new_robot->fiducialmodels.size() == 1)
                new_robot->fiducial_pubs.push_back(n_.advertise<sensor_msgs::FiducialDetection>(mapName(BASE_FIDUCIAL_DETECTION, r, static_cast<Stg::Model*>(new_robot->positionmodel)), 10));
            else
                new_robot->fiducial_pubs.push_back(n_.advertise<sensor_msgs::FiducialDetection>(mapName(BASE_FIDUCIAL_DETECTION, r, s, static_cast<Stg::Model*>(new_robot->positionmodel)), 10));
        }
...
StageNode::WorldCallback()
{
    ...
    for (size_t r = 0; r < this->robotmodels_.size(); ++r)
    {
        ...
        for (size_t s = 0; s < robotmodel->fiducialmodels.size(); ++s)
        {
            ...
            // Translate into ROS message format and publish
            sensor_msgs::FiducialDetection msg;
            msg.angle_min = -fiducialmodel->fov/2.0;
            msg.angle_max = +fiducialmodel->fov/2.0;
            msg.range_min = fiducialmodel->min_range;
            msg.range_max = fiducialmodel->max_range_anon;
            msg.range_max_id = fiducialmodel->max_range_id;
            msg.sigma_radial = 0;
            msg.sigma_polar = 0;
            ...
            msg.sigma_pitch = 0;
            msg.sigma_yaw = 0;
            msg.fiducials.resize(fiducials.size());

            for(unsigned int i = 0; i < fiducials.size(); i++)
            {
                msg.fiducials[i].id = fiducials[i].id;

                msg.fiducials[i].pose.position.x = fiducials[i].range * cos(fiducials[i].bearing);
                msg.fiducials[i].pose.position.y = fiducials[i].range * sin(fiducials[i].bearing);
                msg.fiducials[i].pose.position.z = 0;

                msg.fiducials[i].pose.orientation = tf::createQuaternionMsgFromYaw(fiducials[i].geom.a);
            }
            ...
            // into being retrieved from the param server as a static Tx.
            Stg::Pose fp = fiducialmodel->GetPose();
            tf::Quaternion fiducialQ;
            fiducialQ.setRPY(0.0, 0.0, fp.a);
            tf::Transform txFiducial =  tf::Transform(fiducialQ, tf::Point(fp.x, fp.y, robotmodel->positionmodel->GetGeom().size.z + fp.z));
